...
% Initialize silhouette scores cell array
silhouette_scores = cell(length(k_values), 1);
...
for k_index = 1:length(k_values)
    ...
    silhouette_scores{k_index} = silhouette(nodes, idx);
    
    % Plot silhouette scores for each cluster
    figure;
    silhouette_vals = silhouette(nodes, idx);
    silhouette_groups = splitapply(@(x){x}, silhouette_vals, idx);
    num_clusters = max(idx);
    for i = 1:num_clusters
        subplot(num_clusters, 1, i);
        histogram(silhouette_groups{i}, 'Normalization', 'probability');
        xlabel('Silhouette Score');
        ylabel('Frequency');
        title(['Cluster ' num2str(i)]);
        grid on;
    end
    sgtitle(['Silhouette Analysis for k = ' num2str(k)]);
end
